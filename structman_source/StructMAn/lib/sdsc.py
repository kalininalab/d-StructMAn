...
class Protein:
    __slots__ = ['u_ac','u_id','ref_ids','pdb_id','positions','res_id_map','sequence','stored','go_terms','pathways','disorder_regions','disorder_tool','database_id','structure_annotations']
...
class Proteins:
    __slots__ = ['protein_map','stored_ids','not_stored_ids','id_map','structures','complexes','lite']

    def __init__(self,proteins, lite = False):
    ...
    def get_residue_centralities(self,pdb_id,chain,res_nr):
        return self.structures[(pdb_id,chain)].get_residue_centralities(res_nr)
    ...
    def get_residue_interaction_profile(self,pdb_id,chain,res_nr):
        return self.structures[(pdb_id,chain)].get_residue_interaction_profile(res_nr)
    ...
    def set_stored_ids(self,value):
        self.stored_ids = value
        return
        ...
        for chain in chains:
            ...
            for u_ac in mapped_proteins:
                ...
                for aac_base in sub_infos:
                    ...
                    target_dict[chain][res_nr].append((u_ac,aac_base))
        return target_dict
...
class Structure_annotation:
    ...
    def __init__(self,u_ac,pdb_id,chain,alignment = None,stored = False):
        ...
        self.sub_infos = {} #{aac-base:(res_nr,res_aa)}
        self.stored = stored
        self.database_id = None
...
class Structure:
    ...
    def get_residue_centralities(self,res_nr):
        return self.residues[res_nr].get_centralities()
    ...
    def get_residue_interaction_profile(self,res_nr):
        return self.residues[res_nr].get_interaction_profile()
...
class Mappings:
    __slots__ = ['qualities','covs','seq_ids','rsas','ssas','lig_dists','metal_dists','ion_dists','chain_dists','rna_dists','dna_dists','homo_dists',
                'profiles','weighted_profile','centralities','weighted_centralities','b_factors','weighted_b_factor','modres','weighted_modres','weighted_ssa',
                ...
                'rin_class','rin_simple_class','Class','simple_class','classification_conf','max_cov_rsa']
    ...
    def weight_all(self,config,disorder_score,disorder_region):
        ...
        self.rin_class,self.rin_simple_class = self.weighted_profile.getClass()
    ...
    def get_recommended_res_str(self,proteins):
    ...
    def get_max_seq_structure_res_str(self,proteins):
        ...
        resolution = proteins.get_resolution(pdb_id)
        res_aa = proteins.get_residue_aa(pdb_id,chain,res_nr)
        max_seq_structure = '%s:%s %s:%s;%s;%s;%s' % (pdb_id,chain,res_nr,res_aa,str(seq_id),str(cov),str(resolution))
        return max_seq_structure
...
class Residue:
    ...
    def __init__(self,res_num,aa = 'X',lig_dist_str = None,lig_dists = None,chain_dist_str = None,chain_distances = None,RSA = None,
        ...
        if interaction_profile_str != None and interaction_profile == None:
            self.interaction_profile = rin.Interaction_profile(profile_str = interaction_profile_str)

        self.centralities = centralities
        self.centrality_score_str = centrality_score_str
        if centrality_score_str != None and centralities == None:
            self.centralities = rin.Centrality_scores(code_str = centrality_score_str)
        self.modres = modres
