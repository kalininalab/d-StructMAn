...
class Interaction_profile:
    __slots__ = ['mainchain','sidechain','_class','_simple_class']
    def __init__(self,profile_str=None):
        if profile_str == None:
    ...
    def addEdge(self,chaintype,bondtype,interaction_type,score):
        if chaintype == 'mc':
            ...
            return 'Unknown chaintype: %s' % chaintype
    def encode(self):
    ...
    def computeClass(self):
        ...
        i_types = set(['ligand','ion','metal','Protein','DNA','RNA','Peptide'])
...
def getIAmap(interaction_score_file):
    ...
    for line in lines[1:]:
        ...
        if res_name_a in pdbParser.boring_ligands and not res_name_a in pdbParser.threeToOne:
            continue
        if res_name_b in pdbParser.boring_ligands and not res_name_b in pdbParser.threeToOne:
            continue
        ...
        if not interaction_type_a in IAmap[chain_a][res_nr_a][interaction_base_type]:
        ...
        if not interaction_type_b in IAmap[chain_b][res_nr_b][interaction_base_type]:
    ...
    for bondtype in interaction_map[chain][res]:
        ...
        for chaintype in interaction_map[chain][res][bondtype]:
            for (chain_b,res_b) in interaction_map[chain][res][bondtype][chaintype]:
                ...
                error = profile.addEdge(chaintype,bondtype,interaction_type,score)
                if error != None:
